...
public:
    ...
    {
        const QString requestedLanguage = languageCode == "ar" ? "ar" : "en";
        ...
        if (requestedLanguage == "ar") {
            if (m_translator.load(":/i18n/smarthome_ar.qm"))
                m_application->installTranslator(&m_translator);

            m_application->setLayoutDirection(Qt::RightToLeft);
            QLocale::setDefault(QLocale(QLocale::Arabic, QLocale::Egypt));
        } else {
            m_application->setLayoutDirection(Qt::LeftToRight);
        ...
        // Refresh all qsTr() text bindings without restarting the application.
...
int main(int argc, char *argv[])
{
    ...
    application.setOrganizationName("GeddoApps");
    application.setApplicationName("NexaHome");
    ...
    engine.loadFromModule("GeddoSmartHome", "Main");
